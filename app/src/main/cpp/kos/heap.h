...
void* heap_malloc(unsigned long bytes);
void heap_mfree(void* pointer, unsigned long bytes);

// HEAP.c

static unsigned char heap_space[MAX_HEAP_SPACE];
...
void* heap_malloc(unsigned long bytes) {
	frame_set_buffer = heap_fsalloc(bytes / 0x10 + 1);
